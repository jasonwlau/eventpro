...
import joinGroupBtn from "components/features/joinGroupBtn"
...
export default ({
  ...
  const loadData = async () => {
    ...
    console.log(id);
    if (search === false) {
      const events = firebase.firestore().collection("messages");
      events
        .get()
        .then((querySnapshot) => {
          var ndata = [];
  ...
  const updateInput = async (input) => {
    ...
    events.get().then((querySnapshot) => {
      ...
      const filtered = ndata.filter((event) => {
        return event.chatId.toLowerCase().includes(input.toLowerCase());
      });
      setInput(input);
      setData(filtered);
      setSearch(true);
            ...
            {data.map((post, index) => (
              <PostContainer key={index} featured={0}>
                ...
                >
                  ...
                  <Info>
                    <Title>{post.chatId}</Title>
                  </Info>
                  {joinGroupBtn(post.chatId)}
                </Post>
              </PostContainer>
            ))}
          </Posts>
