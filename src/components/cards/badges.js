...
export default (isProfile) => {
  ...
  const loadBadges=async ()=>{
      ...
      query.docs.forEach((doc)=>{
          newData.push(
            {
                imageSrc: badgeOptions[doc.data().index].image,
                title: badgeOptions[doc.data().index].name,
                description: "Assigned By : "+doc.data().senderName,
                locationText: "",
                pricingText: "",
                rating: 4.5,
                eventName: doc.data().meetingTitle
            })
      })

      console.log(newData)
      if(newData.length!=data.length)setData(data => newData)
  }

  loadBadges()

  if(!isProfile)return <div></div>
  return (
    <Container>
      <Content>
        <HeadingWithControl>
        ...
        <CardSlider ref={setSliderRef} {...sliderSettings}>
          {data.map((card, index) => (
            <Card key={index}>
              ...
              <PrimaryButton>{card.eventName}</PrimaryButton>
            </Card>
          ))}
        </CardSlider>
      </Content>
